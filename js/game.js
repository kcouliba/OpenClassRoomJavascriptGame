...
var Game = {
  ...
  */
  movePlayer: function(player, stepx, stepy) {
    ...
    var nextPos = currentPos.add(Position.new(stepx, stepy));
    var grid = this.grid;
  ...
  ** precomputes next player position
  ** returns next computed position
  ** @param id : int
  ** @param stepx : int
  ** @param stepy : int
  ** @return Position
  */
  precomputePlayerMove: function(player, stepx, stepy) {
    var currentPos = Position.clone(player.getPosition());
    var move = Position.new(0, 0);
    var nextPos = currentPos.add(Position.new(stepx, stepy));
    var grid = this.grid;
    ...
    if (stepx < 0) { // moving left
      ...
      while ((currentPos.x + move.x) > nextPos.x) {
        move.x--;
        if (((currentPos.x + move.x) < 0)
        || grid.stateAt((currentPos.x + move.x), currentPos.y) !== CELLSTATE.FREE) { // obstacle or player encountered
          if (DEBUG) {
            console.log("Player " + player.name + " encountered an obstacle at : " + Position.new((currentPos.x + move.x), currentPos.y));
          }
          move.x++;
          break ;
        }
    ...
    if (stepy < 0) { // moving up
      ...
      if (nextPos.y >= 0) {
        while ((currentPos.y + move.y) > nextPos.y) {
          move.y--;
          if (((currentPos.y + move.y) < 0)
          || grid.stateAt(currentPos.x, (currentPos.y + move.y)) !== CELLSTATE.FREE) { // obstacle or player encountered
            if (DEBUG) {
              console.log("Player " + player.name + " encountered an obstacle at : " + Position.new(currentPos.x, (currentPos.y + move.y)));
            }
            move.y++;
            break ;
          }
